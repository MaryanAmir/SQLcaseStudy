...
FROM IPLPlayers)
SELECT *,CAST(100*(Price_in_cr/total_spending)AS decimal(10,2))
FROM TAB2
...
WITH TAB AS (SELECT Team,Price_in_cr, CASE WHEN Price_in_cr > 15  THEN 'high'
...
WITH TAB AS (SELECT  Player,Price_in_cr,Team ,AVG(Price_in_cr)OVER(PARTITION BY Team) AS avg_indicate
FROM IPLPlayers)
SELECT Team,Player,Price_in_cr
FROM TAB
WHERE Price_in_cr>avg_indicate
